...
| Lấy analysis theo message | `GET /api/ai/analysis/message/{messageId}` | `getAIAnalysisByMessageId(messageId)` |
| Lấy analyses theo session | `GET /api/ai/analysis/session/{sessionId}` | `getAIAnalysesBySessionId(sessionId)` |

### 👤 User Management - `userApi.js`

| Mục đích | API Endpoint | Function |
| Lấy responses theo query | `GET /api/ai/responses/query/{queryId}` | `getAIResponsesByQueryId(queryId)` |
| ---------------------------------- | --------------------- | -------------------------------- |
| Lấy thông tin user hiện tại | `GET /api/users/me` | `getProfile()` |
| (Tùy backend) Xóa analysis/response | `-` | `-` |
| Lấy thông tin user theo ID (Admin) | `GET /api/users/{id}` | `getUserById(id)` |
...
| Lọc/list sessions | `GET /api/chat/sessions?userId=&mine=&pageNumber=&pageSize=` | `getChatSessions(params)` |

### ❓ Questions - `questionApi.js`

| Mục đích | API Endpoint | Function |
| Lấy message theo ID | `GET /api/chat/messages/{messageId}` | `getChatMessageById(messageId)` |
| Upload ảnh message | `POST /api/chat/sessions/{sessionId}/messages/upload` | `uploadChatImageMessage(sessionId, file, params)` |
| ------------------------ | ---------------------------- | ---------------------------------- |
| Cập nhật session | `PATCH /api/chat/sessions/{sessionId}` | `updateChatSession(sessionId, sessionData)` |
...
| Lấy consent theo ID | `GET /api/ConsentRecords/{id}` | `getConsentRecordById(id)` |
| Cập nhật consent | `PUT /api/ConsentRecords/{id}` | `updateConsentRecord(id, data)` |
| Xóa consent | `DELETE /api/ConsentRecords/{id}` | `deleteConsentRecord(id)` |
| Mục đích | API Endpoint | Function |
| ------------------------- | ------------------------------------ | ---------------------------------- |
...
| Lấy feedback theo routine | `GET /api/feedbacks/routine/{routineId}` | `getFeedbacksByRoutineId(routineId)` |
| Lấy feedback theo step | `GET /api/feedbacks/step/{stepId}` | `getFeedbacksByStepId(stepId)` |
